  ...
  methods: {
    ...
    async confirmAddress() {
      ...
      try {
        ...
        } else if (arc20.checkSymbol(this.symbol)) {
          if (wallet.extend.ledger) {
            this.rawTx = 'Please confirm tx on your ledger...'
          }
          const token = arc20.getTokenBySymbol(this.symbol)
          const encodedData = arc20.encodeSendData(
            token,
            this.address,
            this.amount
          )
    ...
    async searchAddToken() {
      ...
      try {
        const tokenInfo = await arc20.fetchTokenInfo(this.addTokenAddress)
        ...
        if (
          ...
          e.message === 'this contract is not a arc20 token'
        ) {
    ...
    initTokens() {
      const tokenList = [{ text: 'ALV', value: 'ALV' }]
      arc20.getTokenList().forEach(token => {
