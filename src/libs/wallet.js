...
switch (config.getNetwork()) {
  case 'testnet':
    network = alvey.networks.alvey_testnet
    break
  case 'mainnet':
    network = alvey.networks.alvey
    break
}
...
export default class Wallet {
  ...
  async setInfo() {
    ...
    this.info.arc20 = info.arc20Balances.map(token => {
  ...
  static generateCreateTokenTx(
    ...
    return alvey.utils.buildCreateContractTransaction(
      wallet.keyPair,
      arc20TokenCode + encodedParam.substr(2),
      gasLimit,
      gasPrice,
      fee,
      utxoList
  ...
  static async generateSendToContractTx(
    ...
    return alvey.utils.buildSendToContractTransaction(
      wallet.keyPair,
      contractAddress,
      encodedData,
      gasLimit,
  ...
  static async generateTx(wallet, to, amount, fee, utxoList) {
    ...
    return alvey.utils.buildPubKeyHashTransaction(
      wallet.keyPair,
      to,
      amount,
      fee,
  ...
  static restoreFromMnemonic(mnemonic, password) {
    ...
    const hdNode = alvey.HDNode.fromSeedHex(seedHex, network)
  ...
  static restoreFromWif(wif) {
    return new Wallet(alvey.ECPair.fromWIF(wif, network))
  }

  static async restoreHdNodeFromLedgerPath(ledger, path) {
    const res = await ledger.alvey.getWalletPublicKey(path)
    const compressed = ledger.alvey.compressPublicKey(
      Buffer.from(res['publicKey'], 'hex')
    )
    const keyPair = new alvey.ECPair.fromPublicKeyBuffer(compressed, network)
    const hdNode = new alvey.HDNode(
      keyPair,
